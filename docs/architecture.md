...
- Apply run-time targeting policy:
  - `--server` sets default host for TCP/UDP entries missing `host` (default `127.0.0.1`)
  - `--force-server` overrides all TCP/UDP `host` values for the run
...
- Execute tests:
  ...
  - HTTP: POST /upload with `X-PSK` + synthetic payload → parse response
- User experience:
...
- Compute minimal metrics:
  ...
  - Responds with JSON status objects
...
- `--manage-firewall`: creates inbound allow rules for configured TCP/UDP/HTTP ports
