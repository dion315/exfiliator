#!/usr/bin/env python3
"""
Purple-team port/protocol test server (TCP/UDP + optional HTTP) with PSK auth.
...
- Uses a pre-shared key (PSK) to prevent abuse:
    - Server generates PSK on first run and stores it in a file
    - Client must present PSK for TCP/UDP/HTTP tests

No file IO beyond PSK storage; no covert channels; no obfuscation.
"""
...
def load_or_create_psk(psk_file: str, print_psk: bool) -> str:
    if os.path.exists(psk_file):
        with open(psk_file, "r", encoding="utf-8") as f:
            psk = f.read().strip()
            if not psk:
                raise ValueError(f"PSK file exists but is empty: {psk_file}")
            log.info("Loaded PSK from: %s", psk_file)
            return psk

    psk = base64.urlsafe_b64encode(os.urandom(32)).decode("utf-8").rstrip("=")

    # Ensure parent dir exists (best-effort)
    parent = os.path.dirname(os.path.abspath(psk_file))
    ...
    # Best-effort permission hardening (POSIX)
    try:
        os.chmod(psk_file, 0o600)
    except Exception:
        pass

    log.info("Generated new PSK and stored at: %s", psk_file)
    if print_psk:
        log.warning("Generated PSK (store securely): %s", psk)
    return psk


def psk_ok(provided: Optional[str], expected: str) -> bool:
    if not provided:
        return False
    return hmac.compare_digest(provided.strip(), expected)


def tcp_worker(bind_ip: str, port: int, recv_timeout_s: float, expected_psk: str) -> None:
    """Simple TCP sink with small ACKs back to client. Requires PSK in JSON header."""
    ...
    while True:
        ...
        try:
            ...
            if not psk_ok(meta.get("psk"), expected_psk):
                raise PermissionError("Invalid PSK")
            ...
            log.info(
                "TCP %s:%d from %s test_id=%s received=%d bytes (%.2f Mbps)",
...
def udp_worker(bind_ip: str, port: int, expected_psk: str) -> None:
    """
    UDP receiver. Requires PSK in HELLO JSON.

      - first packet: b'HELLO ' + JSON {"test_id": "...", "psk": "..."}
      - data packets: b'DATA ' + 8-byte seq + payload
    Responds with:
      - b'HELLO_ACK'
      - ACK packets: b'ACK ' + 8-byte seq
    """
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
    sock.bind((bind_ip, port))
    log.info("UDP listening on %s:%d", bind_ip, port)

    authorized = set()  # set of addr tuples that sent valid HELLO
    ...
    while True:
        ...
        if pkt.startswith(b"HELLO "):
            try:
                ...
                if not psk_ok(meta.get("psk"), expected_psk):
                    log.warning("UDP invalid PSK from %s:%d on port %d", addr[0], addr[1], port)
                    continue

                authorized.add(addr)
                test_id = meta.get("test_id", "unknown")
                log.info("UDP HELLO %s:%d from %s test_id=%s", bind_ip, port, addr[0], test_id)
                sock.sendto(b"HELLO_ACK", addr)
            except Exception as e:
                log.warning("UDP HELLO parse error from %s: %s", addr, e)
            continue

        # Drop data from unauthenticated sources
        if addr not in authorized:
            if log.isEnabledFor(logging.DEBUG):
                log.debug("UDP drop unauthenticated from %s:%d on port %d", addr[0], addr[1], port)
            continue
...
class UploadHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # PSK check
        provided = self.headers.get("X-PSK", "")
        if not psk_ok(provided, self.server.expected_psk):  # type: ignore[attr-defined]
    ...
    def log_message(self, fmt, *args):
        # suppress default noisy http.server logging
        return


def http_worker(bind_ip: str, port: int, expected_psk: str) -> None:
    httpd = HTTPServer((bind_ip, port), UploadHandler)
    httpd.expected_psk = expected_psk  # attach attribute for handler
...
def main():
    ...
    ap.add_argument("--print-psk", action="store_true", help="Print generated PSK to console (use carefully)")
    ...
    psk = load_or_create_psk(args.psk_file, args.print_psk)
    ...
    for p in tcp_ports:
        t = threading.Thread(
            target=tcp_worker,
            args=(args.bind, p, args.tcp_timeout, psk),
            daemon=True,
        )
        t.start()
    ...
    if args.http_port:
        t = threading.Thread(
            target=http_worker,
            args=(args.bind, args.http_port, psk),
            daemon=True,
        )
        t.start()
